...
# python3 tools/test.py ./projects/configs/vma_trunk_line_two_map_swin_later_fusion_att_gram_loss.py  /homes/zhangzijian/vma-dev/new_data_train_result/res_swin_later_fusion_spatial_gram_loss_res34_debug_pt2line10/epoch_48_stable.pth --eval chamfer --eval-option show=True show_dir=three_map_visualize_result_fuse
log_config = dict(
    interval=50,
    hooks=[
...
log_level = 'INFO'
work_dir = "./new_data_train_result/later_fusion_spatial_gram_loss_000_res_swin_debug_pt2line12pt2pt5"
load_from = None
resume_from = None
workflow = [('train', 1)]
...
model = dict(
    ...
    img_backbone=dict(
        ...
        # 关键修改：调整位置编码
        use_abs_pos_embed=False,  # 改为False
        # 关键修改：添加多卡训练相关配置
    ...
    pts_bbox_head=dict(
        ...
        loss_pts2lines=dict(type='Pts2LinesLoss', 
                      loss_weight=12), # 改成12 增大这个损失 
        loss_pts2pts=dict(type='PtsL1Loss', 
                      loss_weight=5.0), # 点到点 改成5 
