...
log_level = 'INFO'
work_dir = "./new_data_train_result/"
load_from = None
resume_from = None
workflow = [('train', 1)]
...
model = dict(
    ...
    #     style='pytorch'),
    img_backbone=dict(
        type='SwinTransformer',  # Swin 骨干网络类型
        pretrained='ckpts/swin_base_patch4_window7_224.pth',  # Swin-B 预训练权重（需自行下载）
        embed_dims=128,  # 基础嵌入维度（Swin-B 标准值）
        depths=[2, 2, 18, 2],  # 各阶段的 Transformer 层数（Swin-B 标准配置）
        num_heads=[4, 8, 16, 32],  # 各阶段的注意力头数（与通道数匹配）
        window_size=7,  # 窗口注意力的窗口大小
        mlp_ratio=4,  # MLP 隐藏层维度比例
        qkv_bias=True,  # QKV 偏置（提升性能）
        drop_rate=0.1,  #  dropout 比例
        attn_drop_rate=0.1,  # 注意力 dropout 比例
        drop_path_rate=0.2,  # 路径 dropout 比例（正则化）
        patch_norm=True,  # Patch 归一化（稳定训练）
        out_indices=(1, 2, 3),  # 输出 3个尺度（对应原 ResNet 的 1,2,3 索引）
        frozen_stages=1,  # 冻结第1阶段参数（与原 ResNet 一致，保证预训练权重复用）
        norm_cfg=dict(type='LN', requires_grad=True),  # Swin 用 LayerNorm（替换 ResNet 的 BN）
        use_abs_pos_embed=True,  # 使用绝对位置嵌入
        act_cfg=dict(type='GELU')  # 激活函数（Swin 标准用 GELU）
    ),
    # fuse_encoder=dict(
    #     type='SimpleFusionEncoder',
    #     in_channels=6,  # 假设img和view_img都是3通道
