...
log_level = 'INFO'
work_dir = "./new_data_train_result/fpn_neck_result"
load_from = None
resume_from = None
workflow = [('train', 1)]
...
model = dict(
    ...
    img_neck=dict(
        type='FPN',
        in_channels=[512, 1024, 2048],  # 输入特征通道（与backbone输出对应）
        out_channels=256,               # 输出特征通道
        num_outs=4,                     # 输出特征层数
        start_level=0,                  # 从第0层开始融合
        add_extra_convs='on_output',    # 补充额外尺度的方式
        norm_cfg=dict(type='GN', num_groups=32),  # 归一化配置
        relu_before_extra_convs=True    # 补充尺度前是否用ReLU
),
    pts_bbox_head=dict(
        type='VMAHead',
        num_query=900,
