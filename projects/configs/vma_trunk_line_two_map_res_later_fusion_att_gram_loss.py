...
bev_res = 0.05  # 0.05米/像素（关键：用于像素转米）
crop_size = (1000, 1000)
...
# attr_classes_num = [8, 6, 3, 7, 3]

fixed_ptsnum_per_gt_line = 25 # 训练时GT的固定点数（模型输入仍需固定长度）
fixed_ptsnum_per_pred_line = 25 # 模型预测的固定点数（推理时再动态插值）
# 动态采样配置（推理时用）
dynamic_sample_config = dict(
    sample_density=2.0,  # 每2米1个点
    min_sample_points=5,  # 最小点数
    max_sample_points=25, # 最大点数
    bev_res=bev_res       # 像素转米的分辨率
)
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        num_pts_per_vec=fixed_ptsnum_per_pred_line, # 模型预测仍输出25个点（推理时动态插值）
        num_pts_per_gt_vec=fixed_ptsnum_per_gt_line, # 训练时GT固定25个点
        dir_interval=1,
...
train_pipeline = [
    ...
    dict(
        ...
        fixed_ptsnum_per_line=fixed_ptsnum_per_gt_line, # 训练时GT固定25个点
    ),
...
data = dict(
    ...
    train=dict(
        ...
        datasets=[
            ...
            dict(
                type=dataset_type,
                data_root=data_root2["train"],
                imgs_dir="cropped_data/images",  # 根据new_data的实际目录结构调整
                annots_dir="cropped_data/annots",# 根据new_data的实际目录结构调整
                mask_dir=None,
                points_nums=fixed_ptsnum_per_gt_line,
                modality=input_modality,
                view_img_dir="cropped_data/view_maps",# 根据new_data的实际目录结构调整
                z_map_dir = "cropped_data/z_maps_vis",# 根据new_data的实际目录结构调整
                map_classes=map_classes,
                attrs_dict=attrs_dict, 
                eval_use_same_gt_sample_num_flag=eval_use_same_gt_sample_num_flag,
                dynamic_sample_config=dynamic_sample_config,
