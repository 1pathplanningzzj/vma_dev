...
class VMAHead(DETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 loss_slope=dict(type='MSELoss', loss_weight=1.0), # 斜率 
        ...
        self.loss_slope = build_loss(loss_slope)
...
# 增加斜率损失 
    def fit_slope(points):
            """辅助函数：对单个向量的点进行线性拟合，返回斜率k"""
        x = points[..., 0]  # 所有点的x坐标，shape [num_samples, num_pts]
        y = points[..., 1]  # 所有点的y坐标，shape [num_samples, num_pts]
            
            # 最小二乘法拟合：k = (nΣxy - ΣxΣy) / (nΣx² - (Σx)²)
        n = x.shape[1]  # 每个向量的点数量（50）
        sum_x = x.sum(dim=1, keepdim=True)  # [num_samples, 1]
        sum_y = y.sum(dim=1, keepdim=True)  # [num_samples, 1]
        sum_xy = (x * y).sum(dim=1, keepdim=True)  # [num_samples, 1]
        sum_x2 = (x ** 2).sum(dim=1, keepdim=True)  # [num_samples, 1]
            
            # 避免分母为0
        denominator = n * sum_x2 - sum_x **2 + 1e-8
        slope = (n * sum_xy - sum_x * sum_y) / denominator
        return slope.squeeze(1)  # 压缩为 [num_samples]

        # 1. 提取预测点和真实点（shape均为 [num_samples, 50, 2]）
        # num_samples = 总向量数（批量内所有样本的向量总和）
    pts_preds_slope = pts_preds  # 已调整为与gt相同的点数量（50）
    pts_gt_slope = normalized_pts_targets.reshape(-1, self.num_pts_per_gt_vec, 2)

        # 2. 计算预测斜率和真实斜率
    slope_pred = fit_slope(pts_preds_slope)  # [num_samples]
    slope_gt = fit_slope(pts_gt_slope)      # [num_samples]

        # 3. 斜率损失的权重（每个向量的平均点权重）
    slope_weights = pts_weights.reshape(-1, self.num_pts_per_gt_vec, 2).mean(dim=[1,2])  # [num_samples]

        # 4. 计算斜率损失
    loss_slope = self.loss_slope(
            slope_pred,
            slope_gt,
            weight=slope_weights,
            avg_factor=num_total_pos
        )
    ...
    if digit_version(TORCH_VERSION) >= digit_version('1.8'):
        ...
        loss_slope = torch.nan_to_num(loss_slope)  # 新增：处理斜率损失的nan

    return loss_cls, loss_pts2lines, loss_pts2pts, loss_dir, loss_attr, loss_offset, loss_slope
    ...
    def loss(self,
        ...
        losses_cls, losses_pts2lines, losses_pts2pts, losses_dir, losses_attr, losses_offset, losses_slope = multi_apply(
        ...
        loss_dict['loss_slope'] = losses_slope[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_attr_i, loss_pts2lines_i, loss_pts2pts_i, loss_dir_i ,loss_offset_i,loss_slope_i in zip(losses_cls[:-1],
                                                                            losses_attr[:-1],
                                                                            losses_pts2lines[:-1],
                                                                            losses_pts2pts[:-1],
                                                                            losses_dir[:-1],
            ...
            loss_dict[f'd{num_dec_layer}.loss_slope'] = loss_slope_i
