...
class VMAHead(DETRHead):
    ...
    def __init__(self,
                 *args,
                 with_box_refine=False, # 迭代优化参考点
                 as_two_stage=False,    #  两阶段
                 transformer=None,   
                 bbox_coder=None,
                 num_cls_fcs=2,
                 num_vec=20,           # 预测向量的数量
                 num_pts_per_vec=2,    #　每个向量多少个点
                 ...
                 loss_pts2lines=dict(type='ChamferDistance', 
                             loss_src_weight=1.0, 
                             loss_dst_weight=1.0), # 点到线段损失
                 loss_pts2pts=dict(type='ChamferDistance', 
                             loss_src_weight=1.0, 
                             loss_dst_weight=1.0), # 点到点的损失
                 loss_dir=dict(type='PtsDirCosLoss', loss_weight=2.0), # 方向损失
                 loss_attr=dict(
                    type='FocalLoss',
                    use_sigmoid=True,
                    gamma=2.0,
                    alpha=0.25,
                    loss_weight=2.0), # 属性损失 
                 loss_offset=None,
                 **kwargs):
    ...
    def forward(self, 
        ...
        if self.query_embed_type == 'all_pts':
            ...
            object_query_embeds = (pts_embeds + instance_embeds).flatten(0, 1).to(dtype) # 广播机制 实例和点的嵌入 
