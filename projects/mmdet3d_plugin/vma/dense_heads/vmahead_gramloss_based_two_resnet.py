    ...
    def forward(self, 
        ...
        if mlvl_feats_view is not None:
            ...
            for feat_pc, feat_img in zip(mlvl_feats, mlvl_feats_view):
                ...
                feat_pc_reshaped = feat_pc.permute(0, 2, 3, 1).reshape(bs, -1, 256)  # [bs, H*W, 256]
                feat_img_reshaped = feat_img.permute(0, 2, 3, 1).reshape(bs, -1, 256)  # [bs, H*W, 256]
                # 计算GramLoss（让主分支模仿View分支的关联模式，或双向对齐）
                gram_loss += self.gram_loss(feat_pc_reshaped, feat_img_reshaped)
            ...
            gram_loss = gram_loss / len(mlvl_feats) * self.gram_loss_weight  # 新增 gramloss的尝试
        ...
        # 打印原始形状，确认是否为[num_query, 2c]（[2500, 512]）
